...
export const ConditionalLogic = ( { defaultValues } ) => {
	...
	return (
		<DivRow
			...
			tooltip={ __( 'Toggle the field group based on other field values', 'meta-box-builder' ) }
		>
...
const Rule = ( { rule, baseName, removeRule } ) => (
	<div className="og-include-exclude__rule og-attribute">
		...
		/>
		<select name={ `${ baseName }[operator]` } className="og-include-exclude__operator" defaultValue={ rule.operator }>
		...
		<input type="text" name={ `${ baseName }[value]` } placeholder={ __( 'Enter a value', 'meta-box-builder' ) } defaultValue={ rule.value } />
