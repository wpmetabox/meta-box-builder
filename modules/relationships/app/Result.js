...
const Result = () => {
	...
	const onClick = () => {
		...
		fetch( `${ MbbApp.rest }/mbb/relationships-generate`, {
			method: 'POST',
			body: formData,
	...
	return <>
		<Input
			...
			defaultValue={ dotProp.get( MbbApp.settings, 'text_domain', 'your-text-domain' ) }
			componentId="text-domain"
		/>
		<Input
			name="settings[function_name]"
			label={ __( 'Function name', 'meta-box-builder' ) }
			defaultValue={ dotProp.get( MbbApp.settings, 'function_name', 'your_prefix_register_relationships' ) }
